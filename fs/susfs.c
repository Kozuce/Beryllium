 ...
 */
static int susfs_handle_event_49(struct fsnotify_group *group,
                                 struct inode *inode,
                                 struct fsnotify_mark *inode_mark,
                                 struct fsnotify_mark *vfsmount_mark,
                                 u32 mask,
                                 void *data,      /* <--- HAPUS 'const' DI SINI */
                                 int data_type,
...
static const struct fsnotify_ops fsnotify_ops = {
    .handle_event = susfs_handle_event_49,
};
...
static int __maybe_unused susfs_sdcard_monitor_fn(void *data)
{
...
void susfs_start_sdcard_monitor_fn(void) {
    /* Bypass monitoring untuk stabilitas Kernel 4.9 */
    SUSFS_LOGI("Bypassing sdcard monitor for Beryllium 4.9 stability\n");
    WRITE_ONCE(susfs_is_sdcard_android_data_decrypted, true);
}

void susfs_init(void) {
    SUSFS_LOGI("susfs is initialized! version: " SUSFS_VERSION "\n");
    /* Pastikan flag ini menyala agar SUSFS aktif tanpa menunggu fsnotify */
    WRITE_ONCE(susfs_is_sdcard_android_data_decrypted, true);
}
