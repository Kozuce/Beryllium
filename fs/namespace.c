...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* Jika ID mount berada dalam rentang SUSFS, gunakan ida SUSFS */
	if (id >= DEFAULT_KSU_MNT_ID) {
		ida_remove(&susfs_mnt_id_ida, id);
		return;
	}

	/* Abaikan unshared mount jika flag ini aktif */
...
static struct mount *susfs_alloc_unshare_ksu_vfsmnt(const char *name, int old_mnt_id)
{
	...
	if (mnt) {
		mnt->mnt_id = old_mnt_id; // Menggunakan ID lama agar tetap konsisten
...
static struct mount *susfs_alloc_non_unshare_ksu_vfsmnt(const char *name)
{
	...
	if (mnt) {
		/* Penyesuaian API IDA untuk Kernel 4.9 */
		if (!ida_pre_get(&susfs_mnt_id_ida, GFP_KERNEL))
			goto out_free_cache;

		res = ida_get_new_above(&susfs_mnt_id_ida, DEFAULT_KSU_MNT_ID, &mnt->mnt_id);
		if (res < 0)
			goto out_free_cache;
