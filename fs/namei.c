...
retry:
	...
	/* Eksekusi lookup real (ke disk) */
	dentry = lookup_real(base->d_inode, dentry, flags);

#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	/* CEGAT DI SINI: Jika hasil dari disk ternyata adalah path yang disembunyikan */
	if (unlikely(dentry) && !IS_ERR(dentry) && dentry->d_inode && !found_sus_path && susfs_is_inode_sus_path(dentry->d_inode)) {
		if (d_in_lookup(dentry))
			d_lookup_done(dentry);

		/* RCU check sesuai standar patch */
		if (!(flags & LOOKUP_RCU))
			dput(dentry);

		/* Ganti dengan dentry palsu agar sistem melaporkan -ENOENT */
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
                if (type == LAST_NORM && nd->path.dentry && nd->path.dentry->d_inode) {
                        if (susfs_is_inode_sus_path(nd->path.dentry->d_inode)) {
                                return -ENOENT;
                        }
                }
#endif
